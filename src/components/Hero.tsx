...
const Hero = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
        ...
        {videos.map((_, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all duration-500 ${
              index === currentVideo
                ? "w-12 bg-gradient-to-r from-secondary to-primary shadow-lg shadow-secondary/30"
      ...
      {/* Content */}
      <div className={`container mx-auto container-padding relative z-10 pt-20 transition-all duration-700 ${
        isTransitioning ? "opacity-0 translate-y-4" : "opacity-100 translate-y-0"
      }`}>
        <div className="max-w-2xl lg:max-w-3xl">
          {/* Badge */}
          <div className="inline-flex items-center gap-3 bg-white/5 backdrop-blur-md border border-white/10 rounded-full px-5 py-2.5 mb-6 animate-fade-up">
          ...
          {/* Headline */}
          <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-display font-bold leading-[1.05] mb-6 animate-fade-up" style={{ animationDelay: '0.1s' }}>
            <span className="text-white drop-shadow-lg">Your Dream Home</span>
            <br />
            <span className="bg-gradient-to-r from-secondary via-secondary to-primary bg-clip-text text-transparent">Starts Here</span>
          </h1>

          {/* Subheadline */}
          <p className="text-base sm:text-lg md:text-xl text-white/70 max-w-xl mb-8 leading-relaxed animate-fade-up" style={{ animationDelay: '0.2s' }}>
          ...
          <div className="flex flex-col sm:flex-row gap-4 animate-fade-up" style={{ animationDelay: '0.3s' }}>
            <Button variant="hero" size="lg" className="group bg-gradient-to-r from-secondary to-primary hover:from-primary hover:to-secondary border-0 shadow-xl shadow-secondary/25">
              View Our Estates
              <ArrowRight className="w-5 h-5 group-hover:translate-x-1 transition-transform" />
            </Button>
            <a href="tel:+2348038034077" className="inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-white/5 backdrop-blur-md border border-white/10 text-white hover:bg-white/10 hover:border-white/20 transition-all duration-300 font-semibold group">
